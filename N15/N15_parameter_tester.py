...
from N15_Newton_Rhapson import *
...
#def parameter_checker(parameter_matrix,ci): #unpack paramteres and test
    ...
    c_set = [[0 for i in range(15)] for j in range(parameter_combos_count)]
    for i in np.arange(0,parameter_combos_count,1): #Begin for loop to test the different model paramters using MOL #Check if you got he upperbound right
        ...
        # [c,whoops,vn_method_of_lines,vn_RJ]=method_of_lines(t,x,y,h,p,tol,fit_coeff,kconv) #Find the concntration profiles in space and time using Method of Lines (MOL)
        ...
        #Unpack the data
        # cim=np.zeros((nx+1,nt)) #Initalize new concentration array where bound and unbound NP concentrations are "unpacked" such that they occupy two different matrices in the same 3-D array 
        # cia=np.zeros((nx+1,nt))
        # cit=np.zeros((nx+1,nt))
        # ctm=np.zeros((nx+1,nt)) #Initalize new concentration array where bound and unbound NP concentrations are "unpacked" such that they occupy two different matrices in the same 3-D array 
        # cta=np.zeros((nx+1,nt))
        # ctt=np.zeros((nx+1,nt))
        xindex=np.arange(0,nx+1)
        yss_guess=np.zeros(2*nx+2)+0.5
        ...
        c_set[i][11]=ctt_ss #Pass along steady-state total nanoparticle concentration
        ...
        # for t_i in tindex:
        #     average_mobile_conc_overtime[t_i]=np.average(ctm[:,t_i])
        #     average_attached_conc_overtime[t_i]=np.average(cta[:,t_i])
