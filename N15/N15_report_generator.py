...
def plot_generator(c_set,parameter_combos_count,parameter_matrix,new_count_number,vn_N2,vn_Main_Code,vn_parameter_matrix_generator,vn_parameter_checker,vn_csv_generator,vn_method_of_lines,vn_RJ,machine_number,internal_export_path,phi_min,phi_max):
    """Static Plotting (Exported to Word Document)"""
    report=docx.Document()
    report.add_heading(f'Results from N15 Run #{new_count_number}-{machine_number}',0)
    now = datetime.now()
    ...
    for pc_i in np.arange(0,parameter_combos_count,1): #Begin for loop to plot the different model paramters using MOL 
        ...
        para3.add_run(f'Minimum interstitial porosity (phi_min): {phi_min}')
        para4=report.add_paragraph(f'Maximum porosity (phi_max): {phi_max}     ')
        ...
        # upper_9= np.amax(cut)*1.1 #Upper bound on mobile total concentration
        # upper_10= np.amax(cat)*1.1 #Upper bound on attached total concentration
        # upper_11= np.amax(ctt)*1.1 #Upper bound on total total concentration
        upper_12= np.amax(cm_ss)*1.1 #Upper bound on mobile total concentration
        upper_13= np.amax(ca_ss)*1.1 #Upper bound on attached total concentration
        upper_14= np.amax(ct_ss)*1.1 #Upper bound on total total concentration
 ...
 #        plt.close()
        ...
        # %%Mobile total
        #tindex_u=np.array([0,5,10,25,50,75,100,125,150,200,250]) for masnual control over timepoints plotted
        # tp_ut=10 #number of time points to plot

        # #Linear discretization of plotted timepionts
        # space_ut=int((nt-1)/tp_ut) #Linear discreitzation of timepoints
        # tindex_ut=np.arange(0,nt,space_ut) #Linear discreitization of timepoints
        # for i_ut in tindex_ut:
        #     cc_ut=cut[:,i_ut]
        #     ti_ut=round(t[i_ut],5)
        #     plt.plot(x,cc_ut,label='t={}'.format(ti_ut))
        # """
        # #Logarthmic discreitzation of plotted timepoints
        # lognt_u=np.log10(nt) #Logarthmic timepoints
        # logspace_u=round((lognt_u)/tp_u,10) #Logarthmic timepoints
        ...
        # plt.xlim(left=0,right=1)
        # plt.ylim(bottom=0,top=upper_9) #Change this
        # plt.xlabel('Position',fontsize=14)
        # plt.ylabel('Dimensionless Concentration',fontsize=14)
        # plt.title('Dimensionless Total Mobile Concentration',fontsize=16)
        # plt.xticks(fontsize=12)
        # plt.yticks(fontsize=12)
        # plt.legend(loc=(0.1,0.1))
        # Mobile_total_filename_partial=f'Mobileplot_total{pc_i}.png'
        # Mobile_total_filename_full=os.path.join(internal_export_path,Mobile_total_filename_partial)
        # plt.savefig(Mobile_total_filename_full)
        # plt.close()
        ...
        # pic4.add_picture(Mobile_total_filename_full, width=docx.shared.Inches(3))
       
        
        # %%Attached  total
 #        tp_bt=10 #number of time points to plot
        

 #        #Linear discretization of plotted timepionts
 #        space_bt=int((nt-1)/tp_bt) #Linear discreitzation of timepoints
 #        tindex_bt=np.arange(0,nt,space_bt) #Linear discreitzation of timepoints
 #        for i_bt in tindex_bt:
 #            cc_bt=cat[:,i_bt]
 #            ti_bt=round(t[i_bt],5)
 #            plt.plot(x,cc_bt,label='t={}'.format(ti_bt))
 #        """
 ...
 #        plt.xlim(left=0,right=1)
 #        plt.ylim(bottom=0,top=upper_10)
 #        plt.xlabel('Position',fontsize=14)
 #        plt.ylabel('Dimensionless Concentration',fontsize=14)
 #        plt.title('Dimensionless Attached Total Concentration',fontsize=16)
 #        plt.xticks(fontsize=12)
 #        plt.yticks(fontsize=12)
 #        plt.legend(loc=(0.1,0.1))
 #        Attached_total_filename_partial=f'Attachedplot_total{pc_i}.png'
 #        Attached_total_filename_full=os.path.join(internal_export_path,Attached_total_filename_partial)
 #        plt.savefig(Attached_total_filename_full)
 #        plt.close()

        
        # %% Total  total
 #        tp_tt=10 #number of time points to plot
        

 #        #Linear discretization of plotted timepionts
 #        space_tt=int((nt-1)/tp_tt) #Linear discreitzation of timepoints
 #        tindex_tt=np.arange(0,nt,space_tt) #Linear discreitzation of timepoints
 #        for i_tt in tindex_tt:
 #            cc_tt=ctt[:,i_tt]
 #            ti_tt=round(t[i_t],5)
 #            plt.plot(x,cc_tt,label='t={}'.format(ti_tt))
 #        """
 ...
 #        plt.xlim(left=0,right=1)
 #        plt.ylim(bottom=0,top=upper_11)
 #        plt.xlabel('Position',fontsize=14)
 #        plt.ylabel('Dimensionless Concentration',fontsize=14)
 #        plt.title('Dimensionless Total Total Concentration',fontsize=16)
 #        plt.xticks(fontsize=12)
 #        plt.yticks(fontsize=12)
 #        plt.legend(loc=(0.1,0.1))
 #        total_total_filename_partial=f'Totalplot_total{pc_i}.png'
 #        total_total_filename_full=os.path.join(internal_export_path,total_total_filename_partial)
 #        plt.savefig(total_total_filename_full)
 #        plt.close()   
        
 #        pics_paragraph3=report.add_paragraph()
 #        pic5=pics_paragraph3.add_run()
 #        pic5.add_picture(Attached_total_filename_full, width=docx.shared.Inches(3))
 #        pic6=pics_paragraph3.add_run()
 #        pic6.add_picture(total_total_filename_full, width=docx.shared.Inches(3))
