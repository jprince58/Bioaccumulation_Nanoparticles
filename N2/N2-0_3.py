...
gam=np.array([0.01,0.1,1,1,100]) #Define dimenionless ratio of diffusivities to test
beta=np.array([0.01,0.1,1,1,100]) #Define the dimensionless ratio of potentials to test
F=np.array([0.01,0.1,1,1,100]) #Define the dimensionless forward reaction rate constant to test
Re=np.array([0.01,0.1,1,1,100]) #Define the dimensionless reverse reaction rate constant to test
...
for pc_i in np.arange(0,parameter_combos_count,1): #Begin for loop to test the different model paramters using MOL #Check if you got he upperbound right
    ...
    para1.add_run(f'Initial time (t1) : {parameter_matrix[pc_i,1]}     ')
    para1.add_run(f'Final time (t2) : {parameter_matrix[pc_i,2]}     ')
    para1.add_run(f'Mesh size (nx) : {parameter_matrix[pc_i,3]}')
    para2=report.add_paragraph(f'Dimensionless ratio of diffusivity (gamma) : {parameter_matrix[pc_i,4]}     ')
    para2.add_run(f'Dimensionless ratio of potential (beta): {parameter_matrix[pc_i,5]}')
    para3=report.add_paragraph(f'Dimensionless forward rate constant (F): {parameter_matrix[pc_i,6]}     ')
    para3.add_run(f'Dimensionless reverse rate constant (R): {parameter_matrix[pc_i,7]}')
    para4=report.add_paragraph('Hill coeffecient (n): %f'%parameter_matrix[pc_i,8])
    ...
    plt.figure(0)
    plt.plot(t,average_conc_overtime,label=f'Gamma={parameter_matrix[pc_i,4]} , Beta={parameter_matrix[pc_i,5]} , F={parameter_matrix[pc_i,6]} , R={parameter_matrix[pc_i,7]} , n={parameter_matrix[pc_i,8]}')
    ...
    plt.figure(1)
    plt.plot(average_conc_overtime,change_in_concentration,label=f'Gamma={parameter_matrix[pc_i,4]} , Beta={parameter_matrix[pc_i,5]} , F={parameter_matrix[pc_i,6]} , R={parameter_matrix[pc_i,7]} , n={parameter_matrix[pc_i,8]}')
...
pic3.add_picture('averageplot.png'%pc_i, width=docx.shared.Inches(3))
pic4=pics_paragraph2.add_run()
pic4.add_picture('changeplot.png'%pc_i, width=docx.shared.Inches(3))
"""
