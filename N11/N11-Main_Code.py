...
if machine_number == 1: #Code to assign the right export paths for each machine, since each is unique
   direct_export_path=r'C:\Users\joshu\Box\Quantum Biofilms\Raw Data\N11_results\Direct Exports'
   internal_export_path=r'C:\Users\joshu\Box\Quantum Biofilms\Raw Data\N11_results\Internal Exports' 
...
import numpy as np
from N11_RJ import *
from N11_method_of_lines import *
from N11_parameter_tester import *
from N11_parameter_matrix import *
from N11_report_generator import *
from N11_csv_generator import *
from N11_linear_fitting import *
...
eps=np.array([1]) #Define dimensionless minimum interstital porosity
...
b=np.array([3]) #Define shape paramter for intersitital porosity profile
c=np.array([1]) #Define shape paramter for traditional proosity profile
phi_min=np.array([0.2]) #Define minimum interstitial porosity
rho=np.zeros(len(phi_min)) #initialize rho vector
for i in np.arange(0,len(rho)):
    rho[i]=phi_min[i]/(1-phi_min[i])*np.exp(b)
Kp=np.array([0.1]) #Define partition coeffecient
