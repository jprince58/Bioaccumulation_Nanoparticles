...
def RJ(x,y,p):
    ...
    for i in index:
        if i==0 :
            ...
            l=int((i-1)/2)
            R[i]=alp*(y[i-1]*(xi*(x[l]**a)-y[i])-K*y[i])
            J[i,i]=-alp*(y[i-1]+K)
            J[i,i-1]=alp*(xi*(x[l]**a)-y[i])
        elif i==ny:
            l=int(i/2)
            R[i]=y[i]-1
            J[i,i]=1;
            ...
            l=int(i/2)
            R[i]=gam*(y[i+2]-2*y[i]+y[i-2])/dx**2-y[i]*alp*(xi*(x[l]**a)-y[i+1])+alp*K*y[i+1]
            J[i,i]=-2*gam/dx**2+gam-alp*(xi*(x[l]**a)-y[i+1])
            J[i,i+2]=gam/dx**2
            J[i,i-2]=gam/dx**2  
            J[i,i+1]=alp*(y[i]+K)
        else:
