...
def plot_generator(c_set,parameter_combos_count,parameter_matrix,new_count_number,vn_N2,vn_Main_Code,vn_parameter_matrix_generator,vn_parameter_checker,vn_csv_generator,vn_method_of_lines,vn_RJ,machine_number,internal_export_path):
    ...
    report.add_heading(f'Results from N10 Run #{new_count_number}-{machine_number}',0)
    now = datetime.now()
    ...
    for pc_i in np.arange(0,parameter_combos_count,1): #Begin for loop to plot the different model paramters using MOL 
        ...
        para2=report.add_paragraph(f'Effective diffusivity (gamma) : {parameter_matrix[pc_i,5]}     ')
        para2.add_run(f'Dimensionless attachment rate constant (alpha): {parameter_matrix[pc_i,6]}')
        para3=report.add_paragraph(f'Dimensionless binding site density (xi): {parameter_matrix[pc_i,7]}     ')
        ...
        para5=report.add_paragraph(f'Dimensionless equilibrium constant (K): {parameter_matrix[pc_i,10]}     ')
